...
def draw_pic():
    ...
    print_params()
    draw_shaft(R)  # 画一个轴
    plt.plot(x_list_light, y_list_light, label='light_object θ={0:.0f}°'.format(theta0*180/np.pi))  # 轻负载坐标绘制
    plt.plot(x_list_heavy, y_list_heavy, label='heavy_object')  # 重负载坐标绘制
    plt.xlabel('x')
    plt.ylabel('y')
    # plt.text(-0.8, -0.3, 'μ=0.1\nR=0.01m\nL=1m\nθ=120°',fontsize=15)
    plt.text(-0.8, -0.4, 'R={0}m\nL=1m\nm=0.1kg\nM=0.2kg\nμ=0.3'.format(R),fontsize=15)
    plt.legend(fontsize=15)
    plt.grid(linestyle='--', color='grey', alpha=0.5)
    plt.title('轻负载位置y-x图',fontsize=15)
    plt.show()
